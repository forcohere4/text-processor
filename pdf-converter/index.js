...
function getColumnWidths(orientation) {
...
async function applyTableStyles(page, orientation) {
...
async function generatePDF(htmlContent, orientation = 'portrait', author = '', isMerge = false) {
    try {
        ...
        // Apply all styles and modifications
        ...
        const pdfOptions = {
            ...
            margin: isMerge 
                ? { top: '0mm', bottom: '11mm', left: '5mm', right: '5mm' }
...
app.post('/generate-pdf', async (req, res) => {
    try {
        const pdfBuffer = await generatePDF(req.body, req.query.orientation, req.query.author, false);
        res.set({
...
app.post('/generate-pdf-merge', async (req, res) => {
    try {
        const pdfBuffer = await generatePDF(req.body, req.query.orientation, req.query.author, true);
        res.set({
            'Content-Type': 'application/pdf',
            'Content-Length': pdfBuffer.length,
        });
